...
$.on('command', function (event) {
    ...
    if (command.equalsIgnoreCase("traffle"))  {
        if (!$.isModv3(sender, event.getTags())) {
            $.say($.getWhisperString(sender) + $.lang.get("net.quorrabot.cmd.modonly"));
            return;
        }
       
        if (args.length == 0) {
            $.say($.getWhisperString(sender) + $.lang.get("net.quorrabot.ticketrafflesystem.usage"));
            return;
        }
        ...
        if (subCommand.equalsIgnoreCase("close") || subCommand.equalsIgnoreCase("end")) {
            ...
            if (Winner == null) {
                $.say($.lang.get("net.quorrabot.ticketrafflesystem.no-winner"));
                $.inidb.RemoveFile('traffleplayer');
                return;
            } else {
                $.say($.lang.get("net.quorrabot.ticketrafflesystem.winner", Winner));
        ...
        if (subCommand.equalsIgnoreCase("repick") || subCommand.equalsIgnoreCase("redraw")) {
            ...
            if (NewWinner == null) {
                $.say($.lang.get("net.quorrabot.ticketrafflesystem.winner-repick"));
                return;
            } else {
                $.say($.lang.get("net.quorrabot.ticketrafflesystem.no-winner-repick", NewWinner));
        ...
        if (subCommand.equalsIgnoreCase("subscriberluck")) {
            if (args.length <= 1 || args[1] > 10) {
                $.say($.getWhisperString(sender) + $.lang.get("net.quorrabot.ticketrafflesystem.sub-luck-usage"));
                return;
            }
            $.inidb.set('settings', 'subscriber_luck', parseInt(args[1]));
            $.say($.getWhisperString(sender) + $.lang.get("net.quorrabot.ticketrafflesystem.sub-luck-set", parseInt(args[1])));
            return;
        }
     
        if (subCommand.equalsIgnoreCase("open") || subCommand.equalsIgnoreCase("start")) {
            if ($.TicketRaffleRunning) {
                $.say($.getWhisperString(sender) + $.lang.get("net.quorrabot.ticketrafflesystem.raffle-already-opened"));
                return;
            }
            ...
            if (args[i] != null && args[i] != undefined && (args[i].equalsIgnoreCase('followers') || args[i].equalsIgnoreCase('(followers)'))) {
                $.Followers = true;
                key = $.lang.get("net.quorrabot.ticketrafflesystem.following2");
                i++;
            } 
            if (args[i] != null && args[i] != undefined && (args[i].equalsIgnoreCase('subscribers') || args[i].equalsIgnoreCase('(subscribers)'))) {
                $.Subscribers = true;
                key = $.lang.get("net.quorrabot.ticketrafflesystem.subscribed");
                i++;
            } 
            if (args[i] != null && args[i] != undefined && !isNaN(args[i])) {
            ...
            if ($.TicketRaffleMaxEntries == 0) {
                $.say($.getWhisperString(sender) + $.lang.get("net.quorrabot.ticketrafflesystem.user-error"));
                return;
            } else if ($.Time >= 1) {
                $.timer.addTimer("./systems/ticketraffleSystem.js", "ttraffle", true, function() {
                    $.timer.clearTimer("./systems/ticketraffleSystem.js", "ttraffle", true);
                    $.say($.lang.get("net.quorrabot.ticketrafflesystem.auto-close2", ($.Time / 2)));
                    return;
                }, ($.Time * 60 * 1000) / 2);
