...
public class HTTPSServer {
    ...
    public HTTPSServer(int port, String oauth, String keystorepath, String keystorepassword, InetAddress ip) {
        ...
        this.httpsFileName = keystorepath;
        this.serverPassword = oauth.replace("oauth:", "");

        Thread.setDefaultUncaughtExceptionHandler(com.gmt2001.UncaughtExceptionHandler.instance());

        try {
            server = HttpsServer.create(new InetSocketAddress(this.ip, this.serverPort), 0);
            ...
            com.gmt2001.Console.out.println("Failed to create a new HTTPS server on port: " + port + ".");
            com.gmt2001.Console.out.println("Please make sure nothing is currently using port " + port + " on your system.");
            com.gmt2001.Console.out.println("You can also change the baseport in the botlogin.txt file if you need port " + port + " for something else.");
    ...
    private void handleIniStore(String uriPath, HttpExchange exchange, Boolean hasPassword) {
        ...
        String[] sections = Quorrabot.instance().getDataStore().GetCategoryList(iniStore);
        String outputString = "";
        ...
        for (String section : sections) {
            ...
            String[] keys = Quorrabot.instance().getDataStore().GetKeyList(iniStore, section);

            for (String key : keys) {
                String value = Quorrabot.instance().getDataStore().GetString(iniStore, section, key);
                outputString += "\r\n" + key + "=" + value;
            }
        }
